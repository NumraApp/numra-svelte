# Contributing to @getnumra/svelte
...
is needed. `test/component.test.js` compiles `RiskBadge.svelte` and renders it
on the server, because this package publishes `.svelte` source and the
merchant's bundler is the thing that compiles it — a template that only works
in the browser would ship unnoticed otherwise.
...
Keep the store a classic store rather than runes. That is what lets one
package serve Svelte 4 and 5, and `package.json` promises `>=4`.
...
React, Vue and Angular packages.
...
- Anything Svelte-shaped — the store contract, `readonlyCheck`, the component,
  the `svelte` export condition — belongs here.
...
Open an issue with the package version, the Svelte version, and the smallest
